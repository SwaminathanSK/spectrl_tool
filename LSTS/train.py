...
from LSTS.PPO import PPO
import gym
# from minigrid.wrappers import FullyObsWrapper, RGBImgObsWrapper
from LSTS.dfa import DFA
...
from spectrl.examples.cartpole import *
...
def make_envs(params):
    env_list_names = "cartpole" #params['training']['envs']
...
def train(params):
    ...
    action_dim_list = []
    for env in envs:
        state_dim = env.system.observation_space.shape
        state_dim_list.append(state_dim)
        # action space dimension
        if has_continuous_action_space:
            action_dim = env.system.action_space.shape[0]
            action_dim_list.append(action_dim)
        else:
            action_dim = env.system.action_space.n-1
            action_dim_list.append(action_dim)
    ...
    while True:
        ...
        while episodes_in_current_iter <= episodes_in_each_iter:
            ...
            for t in range(1, max_ep_len+1):
                ...
                state, reward, terminated, render = env.step([action])
                state = state[0]
                ...
                if timesteps_in_current_iter % print_freq == 0:
                    ...
                    print("Environment: {} \t\t Episode : {} \t\t Timestep : {} \t\t Average Reward : {}".format(current_task, environment_total_episode[0], environment_total_timestep[0]+timesteps_in_current_iter, np.mean(reward_arr[-50:])))
            ...
            if len(overall_performance_done[current_task]) > 100 and np.mean(overall_performance_done[current_task][-100:]) > 0.93:
                ...
                ppo_agent.save(checkpoint_path_list[0])
                is_final_task = dfa_instance.learned_task(current_task)
                is_task_leared =True
                break
            episodes_in_current_iter += 1
            environment_total_episode[0] += 1
            # print_running_episodes += 1
            log_running_reward += current_ep_reward
            log_running_episodes += 1

        environment_total_timestep[0] += timesteps_in_current_iter
    ...
    np.savez_compressed(path_to_save_sunk_timesteps, sunk_timesteps = np.array(environment_total_timestep, dtype="object"))
    np.savez_compressed(path_to_save_sunk_episodes, sunk_episodes = np.array(environment_total_episode, dtype="object"))    
    np.savez_compressed(path_to_save_final_reward, final_reward = np.array(final_task_performance_reward, dtype="object"))
    np.savez_compressed(path_to_save_final_dones, final_dones = np.array(overall_performance_done, dtype="object"))
    np.savez_compressed(path_to_save_final_timesteps, final_timesteps = np.array(overall_performance_timesteps, dtype="object"))
